...
if command -v nwg-displays &>/dev/null || dpkg-query -W -f='${Status}' nwg-displays 2>/dev/null | grep -q "ok installed"; then
  echo -e "${INFO} ${MAGENTA}nwg-displays${RESET} is already installed. Skipping source build."
  exit 0
fi
