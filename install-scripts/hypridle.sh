...
if command -v hypridle &>/dev/null || dpkg-query -W -f='${Status}' hypridle 2>/dev/null | grep -q "ok installed"; then
  echo -e "${INFO} ${MAGENTA}hypridle${RESET} is already installed. Skipping source build."
  exit 0
fi
